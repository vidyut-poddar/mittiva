/* ============================================================
   WebGL Fluid Pointer Effect — websites.html hero only
   Based on Pavel Dobryakov's fluid simulation.
   Adapted for Mittiva:
   - Scoped to the .venom-hero canvas (#fluid) — NOT viewport-wide
   - Vanilla JS (jQuery removed)
   - Skips when running inside an iframe (e.g. shatter preload)
   - Pointer events on the hero element, not the window
   - Coordinates translated to canvas-relative so the splat
     appears under the cursor regardless of scroll/page offset
   ============================================================ */
(function () {
  // Don't run when websites.html is loaded inside the shatter
  // preload iframe — would burn GPU for no reason.
  ...
  function initFluid () {
    ...
    if (!canvas) return;
    // Hero element — used as the pointer-event target so the
    // simulation only responds within the hero.
    ...
    // ── EVENT HANDLERS (scoped to the hero element) ─────────
    // Pointer coordinates are translated from viewport-space to
    // canvas-space via getBoundingClientRect(), so the splat
    // appears under the cursor regardless of scroll or page
    // offset. The animation loop starts on first move (no CPU
    ...
    eventTarget.addEventListener('mousemove', function (e) {
      ...
      if (!running) {
        // First move — initialize pointer + start the loop
        startLoop();
        const color = generateColor();
    ...
    eventTarget.addEventListener('touchstart', function (e) {
      ...
      for (let i = 0; i < touches.length; i++) {
        const pos = getCanvasCoords(touches[i].clientX, touches[i].clientY);
    ...
    eventTarget.addEventListener('touchend', function () {
