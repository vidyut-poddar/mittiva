...
const App = () => {
  ...
  const onStageClick = useCallback((e) => {
    ...
    if (sceneIdx === 0 && !doorPushed) {
      ...
      if (scene0HasPhoto) {
        // Photo present — punch through the doorway with a perspective dolly
        // and warm threshold glow; swap scenes mid-motion for a seamless pass.
        setDollying(true);
        setTimeout(() => setSceneIdx(1), 1000);
        setTimeout(() => setDollying(false), 1700);
      } else {
  ...
  return (
    <div className="stage" onClick={onStageClick}>
      ...
      {/* CSS door overlay only renders when scene 0 has NO dropped photo —
          otherwise the photo carries the entrance on its own and the dolly
          transition does the work. */}
      {sceneIdx === 0 && !scene0HasPhoto && <DoorOverlay open={doorOpen} pushed={doorPushed} />}
